...
struct Song {
    ...
    func isLongerThan(_ song: Song, longest: Int) -> Int{
        
        var newLongest = longest
        if song.duration > longest{
            newLongest = song.duration
        }
...
var initialLongest = songs[0].duration

for song in songs{
    initialLongest = song.isLongerThan(song, longest: initialLongest)
}

print(initialLongest)
/*:
